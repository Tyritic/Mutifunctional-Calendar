...
{
	head = new TeamScheduleNode("", "", "","","",0); // 创建一个空的头结点
	tail = head; // 将尾结点指向头结点
}

void TeamScheduleList::addTeamSchedule(TeamScheduleNode* s)
{
...
void TeamScheduleList::deleteTeamSchedule(string d, string t)
{
    bool found = false; // 标记是否找到要删除的日程
    TeamScheduleNode* p = head; // 定义一个指针p指向头结点
    while (p->next != NULL)
    { // 当p的下一个结点不为空时循环
        if (p->next->date == d && p->next->time == t)
        { // 如果p的下一个结点的日期和时间匹配
            TeamScheduleNode* q = p->next; // 定义一个指针q指向p的下一个结点
...
void TeamScheduleList::modifyTeamSchedule(string d, string t, int i)
{
    bool found = false; // 标记是否找到要修改的日程
    TeamScheduleNode* p = head->next; // 定义一个指针p指向头结点的下一个结点
    while (p != NULL)
    { // 当p不为空时循环
...
void TeamScheduleList::queryTeamSchedule(string d, string t)
{
    ...
    { // 当p不为空时循环
        ...
        { // 如果p所指向的结点的日期和时间匹配
            cout << "查询结果：" << endl;
            cout << "发起人学号/发起人工号：" << p->id << endl;
            cout << "发起人：" << p->name << endl;
            cout << "日期：" << p->date << endl;
            cout << "时间：" << p->time << endl;
...
void TeamScheduleList::showAllTeamSchedules()
{
    ...
    TeamScheduleNode* sorted = new TeamScheduleNode("", "", "","","",0); // 创建一个空的已排序链表的头结点
    TeamScheduleNode* p = head->next; // 定义一个指针p指向头结点的下一个结点
    while (p != NULL) { // 当p不为空时循环
        TeamScheduleNode* q = sorted; // 定义一个指针q指向已排序链表的头结点
        ...
        TeamScheduleNode* r = p->next; // 定义一个指针r指向p的下一个结点
        p->next = q->next; // 将p所指向的结点插入到q所指向的结点后面
        q->next = p; // 更新q的下一个结点为p所指向的结点
        p = r; // 更新p指向r所指向的结点
    }
    TeamScheduleNode* s = sorted->next; // 定义一个指针s指向已排序链表的头结点的下一个结点
    while (s != NULL)
    { // 当s不为空时循环
        cout << "发起人学号/工号：" << s->id << endl;
        cout << "发起人姓名：" << s->name << endl;
...
void TeamScheduleList::clearTeamSchdules()
{
    TeamScheduleNode* p = this->head->next; // 定义一个指针p指向头结点的下一个结点
    while (p != NULL)
    { // 当p不为空时循环
        TeamScheduleNode* q = p->next; // 定义一个指针q指向p的下一个结点
        delete p; // 删除p所指向的结点
